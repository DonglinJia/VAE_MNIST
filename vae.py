...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("Reconstruction shape: " + str(reconsructed_image.shape))
...
def train(model, optimizer, epoch):
    ...
    for batch_index, (data, _) in enumerate(train_loader):
            ...
            loss, tmp_reconstruction_loss = vae_loss_function(recon_batch, data, mu, log_var)
            loss.backward()
            ...
            if batch_index % 100 == 0:
                print('Train Epoch: {} [{}/{} ({:.0f}%)]\t Total Loss: {:.6f} \t Reconstruction Loss: {:.6f}'.format(
                    ...
                    100. * batch_index / len(train_loader), loss.item() / len(data), 
                    tmp_reconstruction_loss.item() / len(data)))

    # avg_train_loss /= len(train_loader)
    # avg_train_reconstruction_loss /= len(train_loader)
    return avg_train_loss / len(train_loader.dataset), avg_train_reconstruction_loss / len(train_loader.dataset)
...
def test(model, epoch):
    ...
    print('\nEpoch {}: Test set: Average loss: {:.6f}, Average reconstruction loss: {:.6f} \n'.format(epoch, 
        avg_test_loss, avg_test_reconstruction_loss))
